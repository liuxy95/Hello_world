...
class Generator:
	...
	def getParameter(self):
		...
		self.f_mem = int(self.f_mem * self.n_regs / tmp_sum);
		self.d_mem = int(self.d_mem * self.n_regs / tmp_sum);
	...
	def genGlobalMemLD_int(self,file_stream,dst):
		file_stream.write(
			'\tld.global.u32	'+dst+', [%s_mem+0];\\n\\\n'
			'\tadd.u64 %s_mem,%s_mem,%distance;\\n\\\n'
		)
	
	def genGlobalMemST_int(self,file_stream,src):
		file_stream.write(
			'\tst.global.u32  [%s_mem+0],'+src+';\\n\\\n'
			'\tadd.u64 %s_mem,%s_mem,%distance;\\n\\\n'
		)
