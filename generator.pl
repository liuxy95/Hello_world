...
while(<STDIN>) {
    ...
    if(/^int_mem\s+((-?\d*)\.?(\d*)(e[+-]\d+)?)/)     { $int_mem = $1;} #if
    if(/^float_mem\s+((-?\d*)\.?(\d*)(e[+-]\d+)?)/)     { $float_mem = $1;} #if
    if(/^strip_pow\s+((-?\d*)\.?(\d*)(e[+-]\d+)?)/)     { $strip_pow = $1;} #if
} #while


#Object is to maximize calories, penalizing heavily for exceeding max cost.
$fitness = compute_fitness($float_alu1,$float_sfu1,$float_mad1,$int_alu1,$int_mad1,$byte_alu1,$loop1,$bbsize1,$seed1,$blockDim1,$gridDim1,$reg_distance1,$sm_distance1,$shared_mem1,$const_mem1,$int_mem,$float_mem,$strip_pow);
...
# ###################################################################
sub compute_fitness{
	my $float_alu,$float_sfu,$float_mad,$int_alu,$int_mad,$byte_alu,$loop,$bbsize,$seed,$blockDim,$gridDim,$reg_distance,$sm_distance,$shared_mem,$const_mem,$int_mem,$float_mem,$strip_pow;
	($float_alu,$float_sfu,$float_mad,$int_alu,$int_mad,$byte_alu,$loop,$bbsize,$seed,$blockDim,$gridDim,$reg_distance,$sm_distance,$shared_mem,$const_mem,$int_mem,$float_mem,$strip_pow)=@_;
	#open(FILE,">>log");
	my $strip = 2**($strip_pow);
	...
	waitpid($pid,0);
	$command="nvcc -o ".$exe."/".$exe." ".$exe."/".$exe.".cu"." -arch sm_20 --compiler-options -fno-strict-aliasing";
	$pid=fork;
	if($pid==0){
		exec($command);
