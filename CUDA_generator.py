...
bbsize = random.randint(50,80)
seed = random.randint(1,10)
b_dim = random.randint(10,128)
g_dim = random.randint(10,200)
r_dist = random.randint(1,8)
sh_mem = random.randint(1,8)
sm_dist = random.randint(1,5)
con_mem = random.randint(1,8)
int_mem = random.randint(0,10)
float_mem = random.randint(0,10)
strip_pow = random.randint(10,20)
...
#command options
options = ' -f ' + prog_name + '.cu' + ' --float_alu=' + '%d'%f_alu + ' --float_sfu=' + '%d'%f_sfu + ' --float_mad=' + '%d'%f_mad + ' --reg_distance=' + '%d'%r_dist + ' --int_alu=' + '%d'%i_alu + ' --int_mad=' + '%d'%i_mad + ' --byte_alu=' + '%d'%b_alu + ' --loop=' + '%d'%loop + ' --bbsize=' + '%d'%bbsize + ' --bbtype=1' + ' --seed=' + '%d'%seed + ' --blockDim=\'' + '%d'%b_dim + ',1,1\'' + ' --gridDim=\'' + '%d'%g_dim + ',1,1\'' + ' --sm_distance=' + '%d'%sm_dist + ' --shared_mem=' + '%d'%sh_mem + ' --const_mem=' + '%d'%con_mem + ' --int_mem=' + '%d'%int_mem + ' --float_mem=' + '%d'%float_mem + ' --strip=' +'%d'%strip
...
comp = os.popen('nvcc -o ' + prog_name + ' ' + prog_name+'.cu -arch sm_20')
print comp.read()

gpgpusim = os.popen('./'+prog_name)
print gpgpusim.read()
